...
namespace Nbi\TranslationBundle\Form\EventListener;
...
class AddTranslatedFieldSubscriber implements EventSubscriberInterface
{
    ...
    public function __construct(
        FormFactoryInterface $factory, ContainerInterface $container, Array $options, $defaultLocale
    )
    {
    ...
    private function bindTranslations($data)
    {
        ...
        foreach ($this->getFieldNames() as $locale => $fieldName) {
            ...
            $collection[] = array(
                'locale' => $locale, 'fieldName' => $fieldName, 'translation' => $Translation,
            );
        }
            ...
            if (NULL === $content && in_array($locale, $this->options['required_locale'])) {
                $form->addError(
                    new FormError(sprintf(
                        "Field '%s' for locale '%s' cannot be blank", $this->options['field'], $locale
                    ))
                );
            } else {
        ...
        foreach ($this->bindTranslations($data) as $binded) {
            ...
            if ($Translation->getId()) {
                ...
                if (!$data->contains($Translation)) {
                    $data->add($Translation, 'textarea');
                }
            }
            if ($binded['locale'] == $this->defaultLocale && NULL != $Translation->getObject()) {
                $setter = 'set' . ucfirst($Translation->getField());
                $Translation->getObject()->$setter($Translation->getContent());
            }
        }
        ...
        foreach ($this->bindTranslations($data) as $binded) {
            $form->add(
                $this->factory->createNamed(
                    ...
                    array(
                        'label' => $binded['locale'],
                        'required' => $this->options['widget'] != 'ckeditor' && in_array($binded['locale'],
                $this->options['required_locale']),
                        'property_path' => false,
                        'attr' => array_merge(array('class' => 'trans-field'), $this->options['attr']),
                    )
                )
            );
        }
